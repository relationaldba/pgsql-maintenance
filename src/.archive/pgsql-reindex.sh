...
################################################################################
#
# Script Name: PostgreSQL reindex databases
#
# Description: The script will run reindex operations on all tables in 
...
function reindex_db() {
    ...
    output=$(reindexdb --dbname="$database" 2>&1)
    return_code=$?
...
do
    ...
    # Vacuum the system catalogs in the database
    # Env variable REINDEX_SYSTEM controls whether system catalogs should be reindexed
    # Use with caution, default value false skips system catalogs
    if [[ $REINDEX_SYSTEM == "true" ]]
    then
        reindex_system $database;
        if [[ $? != 0 ]]
        then
            EXIT_CODE=1
            err "${SCRIPT_NAME}" "System Catalogs failed for database ${database}, see logs above"
        else
            log "${SCRIPT_NAME}" "System Catalogs completed for database ${database}"
        fi
    fi
done
